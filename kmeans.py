...
from numpy.core.fromnumeric import mean  # noqa: E402
...
def get_closest_centroid(point, centroids_dict):
    ...
    """
    c_name = "centroid1"
    compare = euclidean_distance(point, centroids_dict[c_name])
    for key in centroids_dict.keys():
        distance = euclidean_distance(point, centroids_dict[key])
        if distance < compare:
            compare = distance
            c_name = key
    return c_name
...
def update_assignment(list_of_points, centroids_dict):
    ...
    for index in range(len(list_of_points)):
        c_name = get_closest_centroid(list_of_points[index], centroids_dict)
        updt_ctrd[c_name].append(list_of_points[index])
    for keys, value in list(updt_ctrd.items()):
        if value == []:
            del updt_ctrd[keys]
    return updt_ctrd
...
def mean_of_points(list_of_points):
    ...
    result = []
    sum = []
    for sub_index in range(len(list_of_points[0])):
        for index in range(len(list_of_points)):
            sum.append(list_of_points[index][sub_index])
        result.append(mean(sum))
        sum = []
    return result
...
def update_centroids(assignment_dict):
    ...
    for key in assignment_dict.keys():
        updt_ctrd[key] = []
    for key in assignment_dict.keys():
        updt_ctrd[key] = mean_of_points(assignment_dict[key])
    for keys, value in list(updt_ctrd.items()):
        if value == []:
            del updt_ctrd[keys]
    return updt_ctrd
