...
def update_assignment(list_of_points, labels, centroids_dict):
    ...
    for index in range(len(list_of_points)):
        ...
        updt_ctrd[c_name].append(labels[index])
...
def majority_count(labels):
    ...
    for index in range(len(labels)):
        label_counts.append(labels.count(labels[index]))
    max = label_counts[0]
    for index in range(len(label_counts)):
        if max < label_counts[index]:
            max = label_counts[index]
    return max
...
def accuracy(list_of_points, labels, centroids_dict):
    ...
    """
    sum = 0
    label_dict = update_assignment(list_of_points, labels, centroids_dict)
    for key in label_dict.keys():
        label_list = label_dict[key]
        sum = sum + majority_count(label_list)
    result = float(sum / len(labels))
    return result
...
#    There are 9 centroids, probably because there is one centroid that
#    is empty and not shown for us.
...
#    the accuracy would be 0.582. Digit 6 would be easier to be distinguished
#    by the algorithm. I think Digit 9 would be harder, since there are two 9
#    Digits in centroids.
